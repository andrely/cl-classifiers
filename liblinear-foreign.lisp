...
(cffi:defcstruct problem
  ...
  (n :int)
  (y (:pointer :double))
  (x (:pointer (:pointer feature_node)))
  (bias :double))
...
(cffi:defcfun "train" (:pointer model)
  (prob (:pointer problem))
  (param (:pointer parameter)))
...
(defun make-pr ()
  (let ((pr (cffi:foreign-alloc 'problem)))
    ...
    (let ((nodes (cffi:foreign-alloc 'feature_node :count 24))
          (node-p (cffi:foreign-alloc '(:pointer (:pointer feature_node)) :count 5)))
      (setf (cffi:mem-aref node-p '(:pointer (:pointer feature_node)) 0) nodes)

      (let ((indices (loop for i from 0 below 24
                           for (index value) = (elt *test-x* i)
                           for node = (cffi:mem-aref nodes 'feature_node i)
                           do (setf (cffi:foreign-slot-value node 'feature_node 'index) (coerce index 'integer))
                           do (setf (cffi:foreign-slot-value node 'feature_node 'value) (coerce value 'double-float))
                           ; do (setf (cffi:mem-aref nodes 'feature_node i) node)
                           when (= index -1)
                           collect i)))
        (pprint indices)
        (loop for i from 1 below (length indices)
              for j in indices
              do (pprint (list i j))
              do (setf (cffi:mem-aref node-p '(:pointer feature_node) i)
                       (cffi:mem-aref nodes 'feature_node (1+ j))))
