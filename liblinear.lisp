...
(defun predict-values(x model)
  (let* ((x (if (> (model-bias model) 0)
              (append x (list (list (1+ (model-feature-count model)) 1.0) (list -1 0.0)))
              (append x (list (list -1 0.0)))))
         (nr-class (model-class-count model))
         (vals (cffi:foreign-alloc :double :count nr-class))
         (x-mem (cffi:foreign-alloc '%feature-node :count (length x)))
         (pred nil)
         (result nil))
    (%write-feature-nodes x-mem x)
    (setf pred (%predict-values (model-foreign-model model) x-mem vals))
    (setf result (list pred (cffi:convert-from-foreign vals (list :array :double nr-class))))
    (cffi:foreign-free vals)
    (cffi:foreign-free x-mem)
    result))

(defun predict-probability (x model)
  (let* ((x (if (> (model-bias model) 0)
              (append x (list (list (1+ (model-feature-count model)) 1.0) (list -1 0.0)))
              (append x (list (list -1 0.0)))))
         (x-mem (cffi:foreign-alloc '%feature-node :count (length x)))
         (nr-class (model-class-count model))
         (probs (cffi:foreign-alloc :double :count nr-class))
         (pred nil)
         (result (list pred (cffi:convert-from-foreign probs (list :array :double nr-class)))))
    (%write-feature-nodes x-mem x)
    (setf pred (%predict-probability (model-foreign-model model) x-mem probs))
    (setf result (list pred (cffi:convert-from-foreign probs (list :array :double nr-class))))
    (cffi:foreign-free x-mem)
    (cffi:foreign-free probs)
    result))
